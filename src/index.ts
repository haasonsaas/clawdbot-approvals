...
function propose(
  summary: string,
  commands: string[],
  options: { details?: string; expiryMs?: number; env?: Record<string, string>; channel?: string; chatId?: string } = {}
): Approval {
  const now = new Date();
...
function approveAndExecute(id: string): Approval {
  const approved = approve(id);
  return execute(approved.id);
}

function batchApprove(ids: string[] | "all"): { approved: Approval[]; errors: { id: string; error: string }[] } {
